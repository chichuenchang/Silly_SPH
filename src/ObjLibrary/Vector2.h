...
//
	...
	static const vec2 UNIT_X_PLUS;
	static const vec2 UNIT_X_MINUS;
	static const vec2 UNIT_Y_PLUS;
	static const vec2 UNIT_Y_MINUS;
...
//
	...
	vec2 (const vec2& original) : x(original.x),
	                                    y(original.y)
	{}
...
//
	...
	double isNormLessThan (const vec2& other) const
	{
...
//
	...
	double isNormGreaterThan (const vec2& other) const
	{
		return getNormSquared() > other.getNormSquared();
	}
...
//
	...
	bool isAllComponentsLessThan (const vec2& other) const
	{
		assert(isFinite());
...
//
	...
	vec2 getNormalized () const
	{
		...
		return vec2(x * norm_ratio, y * norm_ratio);
	}
...
//
	...
	vec2 getCopyWithNorm (double norm) const
	{
		assert(isFinite());
		assert(!isZero());
		assert(norm >= 0.0);
...
//
	...
	vec2 getCopyWithNormSafe (double norm) const
	{
		assert(isFinite());
		assert(norm >= 0.0);
...
//
	...
	vec2 componentProduct (const vec2& other)
	{
		assert(isFinite());
		assert(other.isFinite());
...
//
	...
	vec2 componentRatio (const vec2& other)
	{
		assert(isFinite());
		assert(other.isFinite());
...
//
	...
	vec2 componentRatioSafe (const vec2& other)
	{
		assert(isFinite());
		assert(other.isFinite());

		return vec2((other.x != 0.0) ? (x / other.x) : x,
...
//
	...
	double getDistanceSquared (const vec2& other) const
	{
		double diff_x = x - other.x;
		double diff_y = y - other.y;
...
//
	...
	bool isDistanceLessThan (const vec2& other,
...
//
	...
	bool isDistanceGreaterThan (const vec2& other,
	                            double distance) const
	{
		return (getDistanceSquared(other) >
		        distance * distance);
...
//
	...
	double getManhattenDistance (const vec2& other) const
	{
...
//
	...
	vec2 projection (const vec2& project_onto) const
	{
		assert(isFinite());
...
//
	...
	double getCosAngle (const vec2& other) const;
...
//
	...
	double getCosAngleSafe (const vec2& other) const;
...
//
	...
	double getAngle (const vec2& other) const;
...
//
	...
	double getAngleSafe (const vec2& other) const;
...
//
	...
	vec2 getRotated (double radians) const
	{
		...
		return vec2(cos_angle * x - sin_angle * y,
...
//
	...
	static vec2 getRandomUnitVector ();
...
//
	...
	static vec2 getClosestPointOnLine (const vec2& l1,
	                                      const vec2& l2,
	                                      const vec2& p,
	                                      bool bounded);
};
